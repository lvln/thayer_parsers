...
for id in "${msgIDs[@]}"; do
        ...
        ./extractonebyid rural.tlog pass.${passSeed} ${id} > /dev/null
				...
				mv ./pass.* ../../mavlink_10_msgs/tests.src/ &> /dev/null
...
make clean > /dev/null
make > /dev/null
./extractbymessageid rural.tlog pass.${passSeed} 0 1 2 4 8 11 20 21 22 23 > /dev/null
mv pass.${passSeed} ../../mavlink_10_msgs/tests.src &> /dev/null
make clean >/dev/null
popd
