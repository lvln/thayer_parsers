...
class CodeGeneratorState(private val root: ElementBase) {
  ...
  // <VALUE> should be a whole-byte length value
  private def xPathLogicHandler(expr: String): String = {
    ...
    if (xPathIfMatcher.matches()) {
      ...
      val compBytes = xPathIfMatcher.group(2).grouped(2).toList // Split hex string into bytes
      ...
      s"""if ($predicate) {
        ...
        if (xPathEqMatcher.matches()) {
          ...
          s"""if ($predicate) {
            ...
            // Match non-hexBinary
            val xPathNonHBIfRegex = """if\(((?:\.\./)+[a-zA-Z0-9_/]+?)eq([0-9]+)\)then(.*)else(.*)"""
            val xPathNonHBIfMatcher = Pattern.compile(xPathNonHBIfRegex).matcher(expr)

            if (xPathNonHBIfMatcher.matches()) {
              val cField = cStructFieldAccess(xPathNonHBIfMatcher.group(1).stripPrefix("../"))
              val compVal = xPathNonHBIfMatcher.group(2)
              val thenOutput = xPathNonHBIfMatcher.group(3)
              val elseOutput = xPathNonHBIfMatcher.group(4)
              ...
              s"""if ($cField == $compVal) {
                ...
                // Match non-hexBinary
                val xPathNonHBEqRegex = """((?:\.\./)+[a-zA-Z0-9_/]+?)eq([0-9]+)"""
                val xPathNonHBEqMatcher = Pattern.compile(xPathNonHBEqRegex).matcher(expr)

                if (xPathNonHBEqMatcher.matches()) {
                  val cField = cStructFieldAccess(xPathNonHBEqMatcher.group(1).stripPrefix("../"))
                  val compVal = xPathNonHBEqMatcher.group(2)
