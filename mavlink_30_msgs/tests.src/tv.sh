...
msgIDs=(0 1 2 4 8 11 20 21 22 23 24 29 30 31 32 33 40 42 43 44 45 46 47 49 51 65 69 73 74 76)
...
for id in "${msgIDs[@]}"; do
        ...
        ./extractonebyid rural.tlog pass.${passSeed} ${id} > /dev/null
				...
				mv ./pass.* ../../mavlink_30_msgs/tests.src/ &> /dev/null
...
make clean > /dev/null
make > /dev/null
./extractbymessageid rural.tlog pass.${passSeed} 0 1 2 4 8 11 20 21 22 23 24 29 30 31 32 33 40 42 43 44 45 46 47 49 51 65 69 73 74 76 > /dev/null
mv pass.${passSeed} ../../mavlink_30_msgs/tests.src &> /dev/null
make clean >/dev/null
popd
