...
* `pass.3` and `pass.n` are all of the individual SCALED_PRESSURE messages fromm two flights with the drone
...
* `fail.3` - `fail.n` are failing tests corresponding to `pass.3` with various fixed fields altered
